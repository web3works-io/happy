...
import { WriteDetailedView, type WriteToolCall } from '@/components/blocks/ToolCalls/Write';
import { BashDetailedView, type BashToolCall } from '@/components/blocks/ToolCalls/Bash';
...
export function MessageDetailView({ message, messageId }: MessageDetailViewProps) {
    ...
    return (
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: safeArea.bottom + 20 }}>
        {tools.map((tool, index) => (
          <View key={index} style={{ marginBottom: index < tools.length - 1 ? 20 : 0 }}>
  ...
  const renderToolFallback = (tool: ToolCall) => (
    <View style={{
      ...
      {tool.result as any && (
        <View>
          <Text style={{ fontSize: 12, color: '#666', marginBottom: 4 }}>Result:</Text>
          <View style={{ backgroundColor: '#fff', padding: 8, borderRadius: 4, borderWidth: 1, borderColor: '#ddd' }}>
            <Text style={{ fontSize: 10, color: '#666', fontFamily: 'monospace' }}>
              {typeof tool.result === 'string' 
                ? tool.result.length > 300 
                  ? tool.result.slice(0, 300) + '...' 
                  : tool.result
                : JSON.stringify(tool.result, null, 2).slice(0, 300) + (JSON.stringify(tool.result).length > 300 ? '...' : '')
              }
            </Text>
          </View>
        </View>
